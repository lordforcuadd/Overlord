    ...
    <div class="flex flex-col lg:flex-row gap-8 items-start lg:items-center justify-between relative z-10">
      <div class="flex-1">
        <div class="flex items-center gap-3 mb-3">
          ...
          <h2 class="text-2xl font-black tracking-tight text-white uppercase">Central de Restauración</h2>
        </div>
        <p class="text-gray-400 text-sm leading-relaxed max-w-2xl">
          El núcleo de Overlord está diseñado para ser seguro, pero puedes forzar la creación de puntos de restauración nativos de Windows, o revertir todos los cambios y volver al estado original de fábrica.
        </p>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-auto shrink-0">
        ...
        >
          ...
          <svg v-else class="animate-spin w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          {{ isReverting ? "RESTAURANDO..." : "RESTAURAR A FÁBRICA" }}
        </button>
      </div>
    </div>
...
const warningMessage = ref(
  "Estás a punto de revertir TODOS los cambios realizados por Overlord. Esto restaurará el estado del Kernel, la telemetría, y las mitigaciones a la configuración original de Windows de forma permanente.<br><br><b>¿Estás completamente seguro de que deseas desarmar Overlord?</b>"
);
